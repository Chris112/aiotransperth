...
- **Train** — plain GETs with static headers (`const.py`), no token. Line/station catalogs have no JSON endpoint; `stations.py` regex-parses the server-rendered dropdowns on the Live Train Times page. `get_train_stations(line)` parses the line-scoped page, whose dropdown is already filtered to that line.

`lines.py` answers "which way is this train going, and does it get me there" — the API never says. It reads `_line_data.py`, a **generated** route-ordered station table; regenerate with `scripts/generate_line_order.py`, which derives order from the fact that a TripId recurs at every station it calls at with increasing times, then asserts every trip is a subsequence of the merged order. Two things there are easy to get wrong: not every line is city-radial (the Airport Line runs *through* the CBD out to Claremont, so nothing may assume the city is at index 0), and departure `Destination` values are bare ("Perth", not "Perth Stn") and may name no station on the line at all — those resolve to the line's own CBD station.
